...
def tiago_arm_link_positions(model, data, q):
    pin.forwardKinematics(model, data, q)
    pin.updateFramePlacements(model, data)
    base_id = model.getFrameId("torso_lift_link")
...
def sample_tiago_arm_points(model, joint_positions):
    data = model.createData()
    return np.asarray([tiago_arm_link_positions(model, data, q) for q in joint_positions], dtype=np.float64)
...
def run_plant(
    ...
    for batch_size in cfg["batch_sizes"]:
        ...
        if plant == "tiago_right":
            stats["arm_points"] = sample_tiago_arm_points(model, np.asarray(stats["joint_positions"], dtype=np.float64))
    ...
    if not no_media:
        ...
        animation_path = save_animation(output_dir, plant, ref_points, results)

    return output_dir, plot_path, animation_path, summaries
...
def save_animation(output_dir, plant, ref_points, results):
    if plant != "tiago_right":
        return None
    ...
    batch_size = sorted(results.keys())[0]
    stats = results[batch_size]
    actual = np.asarray(stats["ee_actual"], dtype=np.float64)[:, :3]
    arm_points = np.asarray(stats["arm_points"], dtype=np.float64)
    timestamps = np.asarray(stats["timestamps"], dtype=np.float64)
    errors = np.asarray(stats["goal_distances"], dtype=np.float64)

    frame_count = min(420, actual.shape[0])
    frame_ids = np.linspace(0, actual.shape[0] - 1, frame_count).astype(np.int64)

    fig = plt.figure(figsize=(8.5, 7.2))
    ax = fig.add_subplot(111, projection="3d")
    ax.plot(ref_points[:, 0], ref_points[:, 1], ref_points[:, 2], linestyle=":", color="#747474", linewidth=1.4, label="reference")
    arm_line, = ax.plot([], [], [], "o-", color="#00693E", linewidth=3.0, markersize=4.0, label="Tiago arm")
    trail_line, = ax.plot([], [], [], color="#003192", linewidth=1.6, alpha=0.85, label="tool trail")
    target_marker, = ax.plot([], [], [], "x", color="#C90016", markersize=8, label="current target")
    tool_marker, = ax.plot([], [], [], "o", color="#111111", markersize=5, label="tool")

    all_points = np.concatenate([arm_points.reshape(-1, 3), actual, ref_points], axis=0)
    center = np.mean(all_points, axis=0)
    radius = max(0.45, float(np.max(np.ptp(all_points, axis=0))) * 0.58)
    ...
    def update(frame_index):
        ...
        target_marker.set_data([ref_points[idx, 0]], [ref_points[idx, 1]])
        target_marker.set_3d_properties([ref_points[idx, 2]])
        tool_marker.set_data([actual[idx, 0]], [actual[idx, 1]])
        tool_marker.set_3d_properties([actual[idx, 2]])
        ax.set_title(f"Tiago transformed figure-8 | t={timestamps[idx]:.2f}s | err={errors[idx]:.3f}m")
        return arm_line, trail_line, target_marker, tool_marker

    anim = animation.FuncAnimation(fig, update, frames=frame_ids.size, interval=50, blit=False)
    gif_path = output_dir / "tracking_arm_3d.gif"
...
def main():
    ...
    output_dir, plot_path, animation_path, summaries = run_plant(
        args.plant,
