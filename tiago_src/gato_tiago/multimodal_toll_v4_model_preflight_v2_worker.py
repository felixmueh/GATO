...
def validate_worker_request(request):
    ...
    reference_size, variant, extension_size_bytes = SUPPORTED_MODULES[module_name]
    ...
    if (
        not isinstance(request.get("extension_size_bytes"), int)
        or request["extension_size_bytes"] <= 0
        or request["extension_size_bytes"] != extension_size_bytes
        or module_path.stat().st_size != extension_size_bytes
    ):
        raise ValueError("worker extension size mismatch")
...
def _run_authorized_worker(request_path, output_path):
    ...
    if not exact_inputs:
        raise ValueError("worker model-preflight-v2 inputs have invalid shape/dtype/domain")
    ...
    summary = {
        ...
        "extension_size_bytes": request["extension_size_bytes"],
