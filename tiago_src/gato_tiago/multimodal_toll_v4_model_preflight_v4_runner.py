...
def _checkpoint(output, generation, stage, rows, arrays, provenance):
    ...
    attempted_worker_modules = [
        row.get("module_name")
        for row in rows
        if isinstance(row.get("identity"), (list, tuple))
        and len(row["identity"]) == 2
        and row["identity"][0] == "worker"
        and row.get("module_name") in {spec["module_name"] for spec in FROZEN_EXTENSIONS}
    ]
    summary = {
        ...
        "completed_worker_count": len(attempted_worker_modules),
        "pending_worker_modules": [
            row["module_name"]
            for row in FROZEN_EXTENSIONS
            if row["module_name"] not in attempted_worker_modules
        ],
...
def _source_provenance(repo):
    ...
    return {
        ...
        "source_hashes_at_start": sources,
        ...
        "accepted_portability_smoke_artifact_loads": 0,
        "accepted_v4_artifact_loads": 0,
...
def authenticate_portability_smoke():
    """Authenticate the accepted smoke before any Pin or worker boundary."""

    for _, (path, digest) in PORTABILITY_SMOKE_PATHS_AND_HASHES.items():
        if not path.is_file() or sha256_file(path) != digest:
            raise RuntimeError("hard-pinned portability smoke path/hash mismatch")
    final_path = PORTABILITY_SMOKE_PATHS_AND_HASHES["final_json"][0]
    npz_path = PORTABILITY_SMOKE_PATHS_AND_HASHES["final_npz"][0]
    manifest_path = PORTABILITY_SMOKE_PATHS_AND_HASHES["final_manifest"][0]
    pointer_path = PORTABILITY_SMOKE_PATHS_AND_HASHES["generation4_pointer"][0]
    summary = json.loads(final_path.read_text())
    manifest = json.loads(manifest_path.read_text())
    pointer = json.loads(pointer_path.read_text())
    with np.load(npz_path, allow_pickle=False) as archive:
        names = tuple(archive.files)
        hashes = {name: array_hash(archive[name]) for name in names}
    expected_extensions = {
        row["module_name"]: row["extension_sha256"] for row in FROZEN_EXTENSIONS
    }
    expected_extension_sizes = {
        row["module_name"]: row["extension_size_bytes"] for row in FROZEN_EXTENSIONS
    }
    smoke_rows = summary.get("rows", ())
    forbidden = summary.get("provenance", {}).get("forbidden_call_counts", {})
    exact = bool(
        summary.get("all_smoke_gates_pass") is True
        and summary.get("incomplete") is False
        and summary.get("certificate", {}).get("all_smoke_gates_pass") is True
        and summary.get("certificate", {}).get("constructor_calls") == 6
        and summary.get("certificate", {}).get("fk_calls") == 6
        and summary.get("certificate", {}).get("cross_module_fk_bitwise_equal")
        is True
        and summary.get("optimization_evidence") is False
        and summary.get("timing_evidence") is False
        and summary.get("provenance", {}).get("extension_hashes")
        == expected_extensions
        and summary.get("provenance", {}).get("extension_sizes")
        == expected_extension_sizes
        and summary.get("provenance", {}).get("frozen_build_head")
        == FROZEN_BUILD_COMMIT
        and summary.get("provenance", {}).get("frozen_cuda_arch")
        == FROZEN_CUDA_ARCH
        and len(smoke_rows) == 3
        and all(row.get("passes") is True for row in smoke_rows)
        and forbidden == EXPECTED_SMOKE_FORBIDDEN_CALL_COUNTS
        and set(names) == set(summary.get("array_names", ()))
        and hashes == summary.get("array_hashes")
        and manifest.get("all_smoke_gates_pass") is True
        and manifest.get("json_sha256")
        == PORTABILITY_SMOKE_PATHS_AND_HASHES["final_json"][1]
        and manifest.get("npz_sha256")
        == PORTABILITY_SMOKE_PATHS_AND_HASHES["final_npz"][1]
        and pointer.get("generation") == 4
    )
    if not exact:
        raise RuntimeError("portability smoke authenticity certificate failed")
    return {
        "all_four_hard_pinned_hashes_match": True,
        "final_metadata_and_arrays_exact": True,
        "portability_smoke_paths_and_hashes": {
            name: {"path": str(path), "sha256": digest}
        ...
        "array_hashes": hashes,
        "extension_hashes": expected_extensions,
...
def certify_worker_module_identity(summary, spec):
    return bool(
        ...
        and summary.get("extension_size_bytes") == spec["extension_size_bytes"]
...
def _validate_worker_boundary(row, spec, input_hash):
    ...
    return bool(
        ...
        and request.get("extension_size_bytes") == spec["extension_size_bytes"]
        ...
        == Path(spec["extension_path"]).resolve()
        and certify_worker_module_identity(summary, spec)
...
def certify_runner_provenance(provenance):
    ...
    return bool(
        ...
        and source_hashes
        == provenance.get("source_hashes_at_start")
        == provenance.get("source_hashes_at_end")
        and all(
        ...
        and provenance.get("accepted_portability_smoke_artifact_loads") == 4
        and provenance.get("accepted_v4_artifact_loads") == 4
...
def _run_pipeline(
    ...
    *,
    ...
    rows = []
    arrays = {}
    _checkpoint(output, 0, "before_smoke_artifact_or_model", rows, arrays, provenance)
    try:
        smoke_authentication = smoke_loader()
        if (
            smoke_authentication.get(
                "all_portability_smoke_authentication_gates_pass"
            )
            is not True
        ):
            raise RuntimeError("portability smoke prerequisite gate failed")
        provenance["portability_smoke_authentication"] = smoke_authentication
        provenance["accepted_portability_smoke_artifact_loads"] = 4
    except BaseException as error:
        rows.append(
            {
                "identity": ["artifact", "portability_smoke"],
                "passes": False,
        ...
        _checkpoint(output, 1, "portability_smoke_failed", rows, arrays, provenance)
        raise
    _checkpoint(output, 1, "portability_smoke_authenticated", rows, arrays, provenance)
    try:
        artifact_arrays, authentication = artifact_loader()
        if (
            authentication.get("all_v4_artifact_authentication_gates_pass")
            is not True
        ):
            raise RuntimeError("V4 artifact prerequisite gate failed")
        ...
        provenance["accepted_v4_artifact_loads"] = 4
    except BaseException as error:
        rows.append(
            {
        ...
        _checkpoint(output, 2, "artifact_failed", rows, arrays, provenance)
        raise
    _checkpoint(output, 2, "v4_artifact_authenticated", rows, arrays, provenance)
    ...
    try:
        ...
        _checkpoint(output, 3, "model_failed", rows, arrays, provenance)
        raise
    _checkpoint(output, 3, "pin_model_preflight_captured", rows, arrays, provenance)

    worker_arrays = {}
    for index, spec in enumerate(FROZEN_EXTENSIONS, start=4):
        try:
            row, captured = worker_launcher(spec, arrays)
            row = dict(row)
            if (
                row.get("module_name") != spec["module_name"]
                or row.get("identity") != ["worker", spec["module_name"]]
            ):
    ...
    _checkpoint(output, 7, "end_provenance_captured", rows, arrays, provenance)
    ...
    certificate["provenance_pass"] = certify_runner_provenance(provenance)
    ...
    summary = {
        ...
        "portability_smoke_authentication": provenance.get(
...
def _production_pipeline(output, *, token=None):  # pragma: no cover
    ...
    def smoke_loader():
        return authenticate_portability_smoke()
    ...
    def worker_launcher(spec, arrays):
        ...
        if (
            ...
            or extension_path.stat().st_size != spec["extension_size_bytes"]
        ):
    ...
    return {
        **frozen_model_preflight_metadata(),
        ...
        "transaction_generations": [
            "gen0_before_smoke_artifact_or_model",
            "gen1_portability_smoke_authenticated",
            "gen2_v4_artifact_authenticated",
            "gen3_pin_model_preflight_captured",
            "gen4_to_gen6_worker_attempts",
            "gen7_end_provenance_captured_before_certificate",
        ],
    }
