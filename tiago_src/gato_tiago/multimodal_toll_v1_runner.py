"""Transactional Stage V1 runner for Tiago tool-center model validation.
...
from gato_tiago.multimodal_toll_v1 import (
...
from gato_tiago.multimodal_toll_v1_worker import (
...
RUNNER_PROTOCOL_VERSION = "tiago_tool_center_toll_v1_runner_1"
AUTHORIZED_OUTPUT_PATH = Path(
    "/tmp/tiago-tool-center-toll-v1-authorized-once/v1.json"
)
...
def repository_root(module_path=Path(__file__)):
    ...
    sentinels = (
        ...
        root / "tiago_src/gato_tiago/multimodal_toll_v1_runner.py",
    )
    if not all(path.exists() for path in sentinels):
        raise RuntimeError("Stage V1 repository root sentinel mismatch")
    return root
...
def certify_runner_provenance(provenance, worker_rows):
    ...
    gates = {
        ...
        "v0_artifacts_excluded": provenance.get("v0_artifacts_consumed") is False
        and provenance.get("v0_rejected_artifact_hashes_report_only")
        == V0_REJECTED_ARTIFACT_HASHES,
    }
...
def _no_existing_artifacts(output):
    ...
    if any(path.exists() for path in candidates):
        raise RuntimeError("Stage V1 does not permit resume, overwrite, or rerun")
...
def _validate_worker_boundary(row, spec, input_npz_sha256):
    ...
    expected_command = [
        ...
        "-B",
        "-m",
        "gato_tiago.multimodal_toll_v1_worker",
        "--request",
        str(request_path),
        "--output",
        str(summary_path),
...
def _run_pipeline(
    ...
    *,
    ...
    certificate = (
        ...
        else {"all_v1_gates_pass": False, "test_override": True}
    )
    exact = (
        ...
        and certificate.get("all_v1_gates_pass") is True
    )
    ...
    summary = {
        "runner_protocol_version": RUNNER_PROTOCOL_VERSION,
        "v1_protocol_version": V1_PROTOCOL_VERSION,
        "incomplete": False,
        "all_v1_gates_pass": all_gates,
        "optimization_evidence": False,
...
def _production_pipeline(output):  # pragma: no cover - execution is blocked
    ...
    if tracked_status_start:
        raise RuntimeError("tracked tree must be clean before any V1 model/task action")
    ...
    provenance = {
        ...
        **exact_invocation_provenance(),
        ...
        "v0_rejected_artifact_hashes_report_only": dict(
            V0_REJECTED_ARTIFACT_HASHES
        ),
    }
    ...
    def final_certifier(task_rows, worker_rows, arrays):
        ...
        if not np.array_equal(
            ...
            return {"all_v1_gates_pass": False, "task_fk_b1_b16_exact": False}
        certificate_rows = []
        ...
        for index, (spec, process_row) in enumerate(zip(extension_specs, worker_rows)):
            ...
            manifest[spec["module_name"].split(".")[-1]] = {
                ...
                "test_command": "python -B -m gato_tiago.multimodal_toll_v1_worker --execute",
        ...
        result["all_v1_gates_pass"] = bool(
            result["all_v1_gates_pass"]
...
def execute_v1_runner(output, *, authorization=None):
    ...
    if (
        ...
        raise RuntimeError("Stage V1 runner execution is blocked")
    if Path(output).resolve() != AUTHORIZED_OUTPUT_PATH:
        raise RuntimeError("Stage V1 runner output is not the single authorized path")
    return _production_pipeline(output)


def describe_v1_runner():
    """Return static protocol metadata without opening any runtime boundary."""
    ...
    return {
        ...
        "artifact_policy": {
            "transactional_generation_zero_before_runtime": True,
            "resume_authorized": False,
            "rerun_authorized": False,
        },
        "extension_binaries_reused_without_build": {
...
def main(argv=None):
    ...
    if not args.execute or RUNNER_EXECUTION_AUTHORIZATION is None:
        raise SystemExit("Stage V1 runner execution is blocked")
    execute_v1_runner(args.output, authorization=RUNNER_EXECUTION_AUTHORIZATION)
    return 0
