                                          ...
                                          T                     ctrl_lim_cost)
{
        ...
        T*                  s_temp = s_reference_traj_k + grid::REFERENCE_SIZE;
        ...
        block::copy<T, grid::REFERENCE_SIZE>(s_reference_traj_k, d_reference_traj_k);
        __syncthreads();

        // cost function
        cost_k =
            plant::trackingcost<T>(STATE_SIZE, CONTROL_SIZE, KNOT_POINTS, s_xux_k, s_reference_traj_k, s_temp, d_robot_model, q_cost, qd_cost, u_cost, N_cost, ee_orient_cost, ee_orient_N_cost, q_lim_cost, vel_lim_cost, ctrl_lim_cost);
        __syncthreads();
...
__host__ size_t getComputeMeritBatchedSMemSize()
{
        size_t size = sizeof(T)
                      * (2 * STATE_SIZE + CONTROL_SIZE + grid::REFERENCE_SIZE +  // reference_traj_k
